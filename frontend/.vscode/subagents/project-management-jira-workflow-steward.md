...
- For a new agent in this repository, prefer `β¨` over `π“` because the change adds a new catalog capability rather than only updating existing documentation
...
|-------------|----------------|----------------|-------------|
| Feature | `feature/JIRA-214-add-sso-login` | `β¨ JIRA-214: add SSO login flow` | New product or platform capability |
| Bug Fix | `bugfix/JIRA-315-fix-token-refresh` | `π› JIRA-315: fix token refresh race` | Non-production-critical defect work |
| Hotfix | `hotfix/JIRA-411-patch-auth-bypass` | `π› JIRA-411: patch auth bypass check` | Production-critical fix from `main` |
| Refactor | `feature/JIRA-522-refactor-audit-service` | `β™»οΈ JIRA-522: refactor audit service boundaries` | Structural cleanup tied to a tracked task |
| Docs | `feature/JIRA-623-document-api-errors` | `π“ JIRA-623: document API error catalog` | Documentation work with a Jira task |
| Tests | `bugfix/JIRA-724-cover-session-timeouts` | `π§ JIRA-724: add session timeout regression tests` | Test-only change tied to a tracked defect or feature |
| Config | `feature/JIRA-811-add-ci-policy-check` | `π”§ JIRA-811: add branch policy validation` | Configuration or workflow policy changes |
| Dependencies | `bugfix/JIRA-902-upgrade-actions` | `π“¦ JIRA-902: upgrade GitHub Actions versions` | Dependency or platform upgrades |
...
- Repository-specific default: use `β¨` when adding a brand-new agent because Gitmoji defines it for new features; use `π“` only when the change is limited to documentation updates around existing agents or contribution docs
...
commit_regex='^(π€|β¨|π›|β™»οΈ|π“|π§|π’„|π”§|π“¦) [A-Z]+-[0-9]+: .+$'
...
## Planned Commits
1. π› JIRA-315: fix refresh token race in auth service
2. π§ JIRA-315: add concurrent refresh regression tests
3. π“ JIRA-315: document token refresh failure modes
